...
*Setup instructions for a Gitian build of gAsp Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the gAsp
...
- [Building gAsp Core](#building-gasp-core)
...
Clone the git repositories for gasp and Gitian.
...
in the gasp repository under 'Fetch and create inputs' to install sources which require
...
To build gAsp Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the gasp repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gasp/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Cryptosio/5b556bea962d7431ce9193d1
...
For example:
```bash
URL=https://github.com/thrasher-/gasp.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit gasp=${COMMIT} --url gasp=${URL} ../gasp/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gasp=${COMMIT} --url gasp=${URL} ../gasp/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gasp=${COMMIT} --url gasp=${URL} ../gasp/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gasp git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gasp/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gasp
SIGPATH=/some/root/path/gasp-detached-sigs

./bin/gbuild --url gasp=${BTCPATH},signature=${SIGPATH} ../gasp/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gasp-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gasp-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gasp-osx-build.assert
```
...
[gasp-project/gitian.sigs.ltc](https://github.com/gasp-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
