...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 11374
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9333
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 21374
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 22485
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 11374
	%{_sbindir}/semanage port -d -p tcp 9333
	%{_sbindir}/semanage port -d -p tcp 21374
	%{_sbindir}/semanage port -d -p tcp 22485
	for selinuxvariant in %{selinux_variants}; do
