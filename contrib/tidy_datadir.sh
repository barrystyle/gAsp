...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete gAsp database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no gAsp datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old gAsp datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected gAsp 0.7 datadir."
    ;;
  3)
    echo "Detected gAsp pre-0.8 datadir."
    ;;
  4)
    echo "Detected gAsp 0.8 datadir."
    ;;
esac
