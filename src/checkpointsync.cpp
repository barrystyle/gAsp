...
bool AcceptPendingSyncCheckpoint()
{
    ...
    if (g_rpc_node->connman && !checkpointMessage.IsNull())
    {
...
bool SendSyncCheckpoint(uint256 hashCheckpoint)
{
    ...
    // Relay checkpoint
    g_rpc_node->connman->ForEachNode([checkpoint](CNode* pnode) {
...
{
    if (g_rpc_node->connman && pfrom->hashCheckpointKnown != hashCheckpoint && pfrom->supportACPMessages)
    {
        pfrom->hashCheckpointKnown = hashCheckpoint;
        g_rpc_node->connman->PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::CHECKPOINT, *this));
    }
}
